...
use Ktpl\CustomerCredit\Block\Creditlimit;
...
class orderSaveAfter implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        try {
            ...
            $order->setState('order_placed_by_credit_limit');
            $order->setStatus('order_placed_by_credit_limit');
            $grand_total = (float)$order->getGrandTotal();
            $payment_method = $order->getPayment()->getMethodInstance()->getTitle();
            $customer_id = $order->getCustomerId();
            $creditLimit = $this->_block->getAvailableLimits($customer_id)->getFirstItem()->getCreditLimit();
            if($payment_method=='Pay By Credit Amount') {
                 ...
                 if($grand_total<=$creditLimit) {
                     ...
                     $objectManager = \Magento\Framework\App\ObjectManager::getInstance();       
                            $question = $objectManager->create('Ktpl\CustomerCredit\Model\Creditlimitreport');
